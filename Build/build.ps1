...
Param(
    [Parameter()]
    [System.String] $script:ModuleName = "HelpDesk",
...
Begin {
    ...
    Function Confirm-VCSModuleNewerThanPublished() {
        ...
        Process {
            $script:VCSModuleVersion = (Test-ModuleManifest -Path (Join-Path $script:MyRoot "$ModuleName.psd1")).Version
            $PublishedModuleVersion = (Find-Module -Repository 'PSGallery' -Name $ModuleName).Version
    ...
    Function Build-Module() {
        ...
        Process {
            $ModuleBuildName = "$ModuleName`_Build"
            $ModuleRoot = Split-Path $PSScriptRoot -Parent
            $ModuleBuildPath = [System.IO.Path]::Combine($ModuleRoot,$ModuleBuildName)
            $ModuleTargetFile = [System.IO.Path]::Combine($ModuleBuildPath,$ModuleName,"$ModuleName.psm1")
            $ManifestPathSource = [System.IO.Path]::Combine($ModuleRoot,"$ModuleName.psd1")
            $ManifestPathTarget = [System.IO.Path]::Combine($ModuleBuildPath,$ModuleName,"$ModuleName.psd1")
            ...
            $null = New-Item -Name "$ModuleName" -Path $ModuleBuildPath -ItemType Directory -Force
    ...
    Function Start-PostBuildTests() {
        ...
        Process {
            $BuildModuleRoot = [System.IO.Path]::Combine($script:MyRoot, "$ModuleName`_Build", $ModuleName)
    ...
    Function Publish-ThisModule() {
        ...
        Process {
            $BuiltModulePath = [System.IO.Path]::Combine($script:MyRoot, "$ModuleName`_Build", $ModuleName)
    ...
    Function Remove-ModuleBuild() {
        ...
        Process {
            ...
            $BuildModulePath = [System.IO.Path]::Combine($script:MyRoot, "$ModuleName`_Build")
    ...
    Function Get-LatestChangeLogSection() {
        ...
        Process {
            ...
            If ($LatestChangeLogVersion -eq $Version) {
                ...
                $null = $Markdown.AppendLine("# Changelog for $ModuleName")
